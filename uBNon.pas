...
begin
 try
  ...
  MessageBox(fBNon.Handle,'Ошибка! Произошла ошибка во время работы программы. Попробуйте закрыть и вновь открыть текущее окно.','Ошибка.',MB_ICONWARNING or MB_OK);
 end;
 FMode := 1;
end;
...
begin
 ...
 begin
  fWait.Close();
  fBNon.Enabled := TRUE;
  MessageBox(fBNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 end else
 begin
//  z := SaveDoc();
  fWait.Close();
  fBNon.Enabled := TRUE;
  if (z = 0) then MessageBox(fBNon.Handle,'Данные успешно сохранены!','Сохранение успешно.',MB_ICONINFORMATION or MB_OK) else
  if (z = 1) then MessageBox(fBNon.Handle,'Во время сохранения данных в Microsoft Word произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 end;
end;
