...
begin
 try
  ...
  if UserID = -1 then
  begin
   MessageBox(fUAuto.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Exit;
  end;
  ...
  end else MessageBox(fUAuto.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 except
  MessageBox(fUAuto.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
  fData.Transaction.Active := FALSE;
  Result := 1;
...
begin
 case OpenMode of
  0: Add();
  1: if Update()=0 then MessageBox(fUAuto.Handle,'Данные успешно сохранены!','Сохранение успешно.',MB_ICONINFORMATION or MB_OK);
 end;
end;
...
begin
 ...
 try
  ...
  Mode := 1;
  MessageBox(fUAuto.Handle,'При чтении данных произошла ошибка.','Ошибка!',MB_ICONWARNING or MB_OK);
 end;
 fUAuto.Caption := 'Клиент. Авто. '+Efam.Text;
end;
