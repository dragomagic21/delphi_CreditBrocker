...
procedure TfUBuild.rghplaceClick(Sender: TObject);
begin
...
procedure TfUBuild.cbloldClick(Sender: TObject);
begin
...
procedure TfUBuild.rghomeClick(Sender: TObject);
begin
...
procedure TfUBuild.rgworktClick(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUBuild.bWorkClick(Sender: TObject);
begin
 Application.CreateForm(TfUWork, fUWork);
 fUWork.UserID   := UserID;
 fUWork.Caption  := 'Работа. '+Efam.Text;
...
procedure TfUBuild.chwpClick(Sender: TObject);
begin
...
procedure TfUBuild.cbshumChange(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUBuild.bShumClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
procedure TfUBuild.rgFamClick(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUBuild.bFamClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
begin
 try
  ...
  { /USER AUTO\}
  rec := ''; val := '';
  rec := 'USR,TYP,TYP_E,TR,COST,FV,TC,TC_ST,TC_AM,TC_EL,SADDCR,PRIM';
  UserID := fData.GetMaxID('TB_USER');
  if UserID = -1 then
  begin
   MessageBox(fUBuild.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Exit;
  end;
  ...
  { /USER BUILD\}
  if fData.CustomAdd('TB_UBUILD',rec,val)=1 then
  begin
   MessageBox(fUBuild.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Result := 1;
  end;
  fData.Transaction.Active := FALSE;
  end else MessageBox(fUBuild.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 except
  MessageBox(fUBuild.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
  fData.Transaction.Active := FALSE;
  Result := 1;
...
procedure TfUBuild.CreateParams(var Params: TCreateParams);
begin
...
begin
 try
  ...
  begin
   rec.Free; val.Free;
   rec:=TStringList.Create; rec.Clear; rec.Add('USR'); rec.Add('TYP'); rec.Add('TYP_E'); rec.Add('TR'); rec.Add('COST'); rec.Add('FV'); rec.Add('TC'); rec.Add('TC_ST'); rec.Add('TC_AM'); rec.Add('TC_EL'); rec.Add('SADDCR'); rec.Add('PRIM'); 
   rec.Add('MN');
   val:=TStringList.Create; val.Clear;
   ...
   if fData.CustomUpd('TB_UBUILD',rec,val,'USR='+IntToStr(UserID))=1 then
   begin
    MessageBox(fUBuild.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
    Result := 1;
   end else
...
procedure TfUBuild.FormCreate(Sender: TObject);
begin
 if (Screen.Height<1024) then
 begin
  fUBuild.AutoSize:=FALSE;
  fUBuild.Width:=700;
  fUBuild.Height:=500;
 end;
 OpenMode := 2;
...
procedure TfUBuild.FormShow(Sender: TObject);
begin
 if (OpenMode = 1) then
 try
  Mode := 0;
  ...
  { /USER\ }
  { \\\___ AUTO ___///}
  rgtyp.ItemIndex := fData.CustomSelI('TB_UBUILD','TYP',' where USR='+IntToStr(UserID));
  if (rgtyp.ItemIndex = 3) then Etyp_e.Text := fData.CustomSelS('TB_UBUILD','TYP_E',' where USR='+IntToStr(UserID));
  rgnb.ItemIndex := fData.CustomSelI('TB_UBUILD','TR',' where USR='+IntToStr(UserID));
  Eacost.Text := fData.CustomSelS('TB_UBUILD','COST',' where USR='+IntToStr(UserID));
  Eafv.Text := fData.CustomSelS('TB_UBUILD','FV',' where USR='+IntToStr(UserID));
  Etc.Text := fData.CustomSelS('TB_UBUILD','TC',' where USR='+IntToStr(UserID));
  case fData.CustomSelI('TB_UBUILD','SADDCR',' where USR='+IntToStr(UserID)) of
   0: chsaddcr.Checked := FALSE;
   1: chsaddcr.Checked := TRUE;
  end;
  case fData.CustomSelI('TB_UBUILD','TC_ST',' where USR='+IntToStr(UserID)) of
   0: chTC_ST.Checked := FALSE;
   1: chTC_ST.Checked := TRUE;
  end;
  case fData.CustomSelI('TB_UBUILD','TC_AM',' where USR='+IntToStr(UserID)) of
   0: chTC_AM.Checked := FALSE;
   1: chTC_AM.Checked := TRUE;
  end;
  case fData.CustomSelI('TB_UBUILD','TC_EL',' where USR='+IntToStr(UserID)) of
   0: chTC_EL.Checked := FALSE;
   1: chTC_EL.Checked := TRUE;
  end;
  Eaprim.Text := fData.CustomSelS('TB_UBUILD','PRIM',' where USR='+IntToStr(UserID));
  cbMN.ItemIndex := fData.CustomSelI('TB_UBUILD','MN',' where USR='+IntToStr(UserID));
  { ///___ AUTO ___\\\}
  Mode := 1;
 except
  Mode := 1;
  MessageBox(fUBuild.Handle,'При чтении данных произошла ошибка.','Ошибка!',MB_ICONWARNING or MB_OK);
 end;
 fUBuild.Caption := 'Клиент. Недвижимость. '+Efam.Text;
end;

procedure TfUBuild.bSaveClick(Sender: TObject);
begin
 case OpenMode of
  0: Add();
  1: if Update()=0 then MessageBox(fUBuild.Handle,'Данные успешно сохранены!','Сохранение успешно.',MB_ICONINFORMATION or MB_OK);
 end;
end;

procedure TfUBuild.cbshum_eClick(Sender: TObject);
begin
...
procedure TfUBuild.chfm_bClick(Sender: TObject);
begin
...
procedure TfUBuild.chfm_aClick(Sender: TObject);
begin
...
procedure TfUBuild.chfm_eClick(Sender: TObject);
begin
...
procedure TfUBuild.chfm_dClick(Sender: TObject);
begin
...
procedure TfUBuild.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 fMain.uBuildShow(self);
end;
