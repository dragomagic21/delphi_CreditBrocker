...
procedure TfUEarth.rghplaceClick(Sender: TObject);
begin
...
procedure TfUEarth.cbloldClick(Sender: TObject);
begin
...
procedure TfUEarth.rghomeClick(Sender: TObject);
begin
...
procedure TfUEarth.rgworktClick(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUEarth.bWorkClick(Sender: TObject);
begin
 Application.CreateForm(TfUWork, fUWork);
 fUWork.UserID   := UserID;
 fUWork.Caption  := 'Работа. '+Efam.Text;
...
procedure TfUEarth.chwpClick(Sender: TObject);
begin
...
procedure TfUEarth.cbshumChange(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUEarth.bShumClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
procedure TfUEarth.rgFamClick(Sender: TObject);
begin
 if (Mode = 1) then
...
procedure TfUEarth.bFamClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
begin
 try
  ...
  { /USER EARTH\}
  rec := ''; val := '';
  rec := 'USR,TYP,TYP_E,COST,FV,TC,TC_ST,TC_AM,TC_EL,PRIM';
  UserID := fData.GetMaxID('TB_USER');
  if UserID = -1 then
  begin
   MessageBox(fUEarth.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Exit;
  end;
                                   val:=val+IntToStr(UserID)+',';
                                   val:=val+IntToStr(rgtyp.ItemIndex)+',';
  if (rgtyp.ItemIndex <> 2)   then val:=val+''''''+',' else
  ...
  { /USER EARTH\}
  if fData.CustomAdd('TB_UEARTH',rec,val)=1 then
  begin
   MessageBox(fUEarth.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Result := 1;
  end;
  fData.Transaction.Active := FALSE;
  end else MessageBox(fUEarth.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 except
  MessageBox(fUEarth.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
  fData.Transaction.Active := FALSE;
  Result := 1;
...
procedure TfUEarth.CreateParams(var Params: TCreateParams);
begin
...
begin
 try
  ...
  begin
   rec.Free; val.Free;
   rec:=TStringList.Create; rec.Clear; rec.Add('USR'); rec.Add('TYP'); rec.Add('TYP_E'); rec.Add('COST'); rec.Add('FV'); rec.Add('TC'); rec.Add('TC_ST'); rec.Add('TC_AM'); rec.Add('TC_EL'); rec.Add('PRIM'); rec.Add('MN');
   val:=TStringList.Create; val.Clear;
   val.Add(IntToStr(UserID));
   val.Add(IntToStr(rgtyp.ItemIndex));
   if (rgtyp.ItemIndex = 2) then val.Add(''''+Trim(Etyp_e.Text)+'''') else val.Add('''''');
   ...
   if fData.CustomUpd('TB_UEARTH',rec,val,'USR='+IntToStr(UserID))=1 then
   begin
    MessageBox(fUEarth.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
    Result := 1;
   end else
...
procedure TfUEarth.FormCreate(Sender: TObject);
begin
 if (Screen.Height<1024) then
 begin
  fUEarth.AutoSize:=FALSE;
  fUEarth.Width:=700;
  fUEarth.Height:=500;
 end;
 OpenMode := 2;
...
procedure TfUEarth.FormShow(Sender: TObject);
begin
 if (OpenMode = 1) then
 try
  Mode := 0;
  ...
  { /USER\ }
  { \\\___ AUTO ___///}
  rgtyp.ItemIndex := fData.CustomSelI('TB_UEARTH','TYP',' where USR='+IntToStr(UserID));
  if (rgtyp.ItemIndex = 2) then Etyp_e.Text := fData.CustomSelS('TB_UEARTH','TYP_E',' where USR='+IntToStr(UserID));
  Eacost.Text := fData.CustomSelS('TB_UEARTH','COST',' where USR='+IntToStr(UserID));
  Eafv.Text := fData.CustomSelS('TB_UEARTH','FV',' where USR='+IntToStr(UserID));
  Etc.Text := fData.CustomSelS('TB_UEARTH','TC',' where USR='+IntToStr(UserID));
  case fData.CustomSelI('TB_UEARTH','TC_ST',' where USR='+IntToStr(UserID)) of
   0: chTC_ST.Checked := FALSE;
   1: chTC_ST.Checked := TRUE;
  end;
  case fData.CustomSelI('TB_UEARTH','TC_AM',' where USR='+IntToStr(UserID)) of
   0: chTC_AM.Checked := FALSE;
   1: chTC_AM.Checked := TRUE;
  end;
  case fData.CustomSelI('TB_UEARTH','TC_EL',' where USR='+IntToStr(UserID)) of
   0: chTC_EL.Checked := FALSE;
   1: chTC_EL.Checked := TRUE;
  end;
  Eaprim.Text := fData.CustomSelS('TB_UEARTH','PRIM',' where USR='+IntToStr(UserID));
  cbMN.ItemIndex := fData.CustomSelI('TB_UEARTH','MN',' where USR='+IntToStr(UserID));
  { ///___ AUTO ___\\\}
  Mode := 1;
 except
  Mode := 1;
  MessageBox(fUEarth.Handle,'При чтении данных произошла ошибка.','Ошибка!',MB_ICONWARNING or MB_OK);
 end;
 fUEarth.Caption := 'Клиент. Земля. '+Efam.Text;
end;

procedure TfUEarth.bSaveClick(Sender: TObject);
begin
 case OpenMode of
  0: Add();
  1: if Update()=0 then MessageBox(fUEarth.Handle,'Данные успешно сохранены!','Сохранение успешно.',MB_ICONINFORMATION or MB_OK);
 end;
end;

procedure TfUEarth.cbshum_eClick(Sender: TObject);
begin
...
procedure TfUEarth.chfm_bClick(Sender: TObject);
begin
...
procedure TfUEarth.chfm_aClick(Sender: TObject);
begin
...
procedure TfUEarth.chfm_eClick(Sender: TObject);
begin
...
procedure TfUEarth.chfm_dClick(Sender: TObject);
begin
...
procedure TfUEarth.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 fMain.uEarthShow(self);
end;
