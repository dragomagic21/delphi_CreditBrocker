...
type
  ...
  public
    ...
    UserType: integer; {авто, недвижимость, земля, ...}
    UserID  : integer;
    { Public declarations }
  end;
...
procedure TfUNon.rghplaceClick(Sender: TObject);
begin
...
procedure TfUNon.cbloldClick(Sender: TObject);
begin
...
procedure TfUNon.rghomeClick(Sender: TObject);
begin
...
procedure TfUNon.rgworktClick(Sender: TObject);
begin
 if (Mode = 1) then
 if ((rgworkt.ItemIndex = 0)or(rgworkt.ItemIndex = 1)) then
 begin
  if (rgFam.ItemIndex <> fData.CustomSelI('TB_USER','WORKT','where ((ID='+IntToStr(UserID)+')and(TUSER='+IntToStr(UserType)+'))')) then
  begin
...
procedure TfUNon.bWorkClick(Sender: TObject);
begin
 Application.CreateForm(TfUWork, fUWork);
 fUWork.UserID   := UserID;
 fUWork.Caption  := 'Работа. '+Efam.Text;
...
procedure TfUNon.chwpClick(Sender: TObject);
begin
...
procedure TfUNon.cbshumChange(Sender: TObject);
begin
 if (Mode = 1) then
 if (cbshum.ItemIndex = 1) then
 begin
  if (cbshum.ItemIndex <> fData.CustomSelI('TB_USER','SHUM','where ((ID='+IntToStr(UserID)+')and(TUSER='+IntToStr(UserType)+'))')) then
  begin
...
procedure TfUNon.bShumClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
procedure TfUNon.rgFamClick(Sender: TObject);
begin
 if (Mode = 1) then
 if ((rgFam.ItemIndex = 1)or(rgFam.ItemIndex = 2)) then
 begin
  if (rgFam.ItemIndex <> fData.CustomSelI('TB_USER','FAMILY','where ((ID='+IntToStr(UserID)+')and(TUSER='+IntToStr(UserType)+'))')) then
  begin
...
procedure TfUNon.bFamClick(Sender: TObject);
begin
 if (Mode = 1) then
 begin
  Application.CreateForm(TfUShum, fUShum);
...
begin
 try
  ...
  { /USER AUTO\}
  rec := ''; val := '';
  rec := 'USR,SUMA,TC,PRIM';
  UserID := fData.GetMaxID('TB_USER');
  if UserID = -1 then
  begin
   MessageBox(fUNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Exit;
  end;
                                   val:=val+IntToStr(UserID)+',';
  if Length(Esuma.Text)>0     then val:=val+''''+Trim(Esuma.Text)+''''+','  else val:=val+''''''+',';
  ...
  { /USER BUILD\}
  if fData.CustomAdd('TB_UNON',rec,val)=1 then
  begin
   MessageBox(fUNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
   Result := 1;
  end;
  fData.Transaction.Active := FALSE;
  end else MessageBox(fUNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
 except
  MessageBox(fUNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
  fData.Transaction.Active := FALSE;
  Result := 1;
...
procedure TfUNon.CreateParams(var Params: TCreateParams);
begin
...
begin
 try
  ...
  begin
   rec.Free; val.Free;
   rec:=TStringList.Create; rec.Clear; rec.Add('USR'); rec.Add('SUMA'); rec.Add('TC'); rec.Add('PRIM'); rec.Add('MN');
   val:=TStringList.Create; val.Clear;
   ...
   if fData.CustomUpd('TB_UNON',rec,val,'USR='+IntToStr(UserID))=1 then
   begin
    MessageBox(fUNon.Handle,'Во время сохранения данных произошла ошибка! Попробуйте повторить позднее.','Ошибка!',MB_ICONWARNING or MB_OK);
    Result := 1;
   end else
...
procedure TfUNon.FormCreate(Sender: TObject);
begin
 if (Screen.Height<1024) then
 begin
  fUNon.AutoSize:=FALSE;
  fUNon.Width:=700;
  fUNon.Height:=500;
 end;
 OpenMode := 2;
...
procedure TfUNon.FormShow(Sender: TObject);
begin
 if (OpenMode = 1) then
 try
  Mode := 0;
  ...
  { /USER\ }
  { \\\___ AUTO ___///}
  Esuma.Text := fData.CustomSelS('TB_UNON','SUMA',' where USR='+IntToStr(UserID));
  Etc.Text := fData.CustomSelS('TB_UNON','TC',' where USR='+IntToStr(UserID));
  Eaprim.Text := fData.CustomSelS('TB_UNON','PRIM',' where USR='+IntToStr(UserID));
  cbMN.ItemIndex := fData.CustomSelI('TB_UNON','MN',' where USR='+IntToStr(UserID));
  { ///___ AUTO ___\\\}
  Mode := 1;
 except
  Mode := 1;
  MessageBox(fUNon.Handle,'При чтении данных произошла ошибка.','Ошибка!',MB_ICONWARNING or MB_OK);
 end;
 fUNon.Caption := 'Клиент. Беззалоговый. '+Efam.Text;
end;

procedure TfUNon.bSaveClick(Sender: TObject);
begin
 case OpenMode of
  0: Add();
  1: if Update()=0 then MessageBox(fUNon.Handle,'Данные успешно сохранены!','Сохранение успешно.',MB_ICONINFORMATION or MB_OK);
 end;
end;

procedure TfUNon.cbshum_eClick(Sender: TObject);
begin
...
procedure TfUNon.chfm_bClick(Sender: TObject);
begin
...
procedure TfUNon.chfm_aClick(Sender: TObject);
begin
...
procedure TfUNon.chfm_eClick(Sender: TObject);
begin
...
procedure TfUNon.chfm_dClick(Sender: TObject);
begin
...
procedure TfUNon.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 fMain.uNonShow(self);
end;
